...
export function buildWebCreatorInstructions(
  ...
  steps.push(
    `Final pass per the playbook, across ALL pages${bilingual ? " in BOTH English and Spanish" : ""}: SEO meta tags + JSON-LD schema + sitemap.xml (include every page URL${bilingual ? ", English and Spanish" : ""}) + robots.txt, adaptive favicons, the OG/social share image, the booking widget embed (if provided), and a 404 page. Generate any needed hero/OG images per the playbook's image rules. Verify every internal link resolves${bilingual ? ", the EN⇄ES language toggle works both ways on every page, and hreflang/canonical tags are correct" : ""}, the no-pricing and contact-display rules hold on every page, and the site is deployable as a static site. Fix anything broken, then commit and push.`,
  );
