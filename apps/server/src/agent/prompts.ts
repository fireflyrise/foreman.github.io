...
export function buildWebCreatorInstructions(
  ...
  const steps: string[] = [
    ...
    `Build a dedicated page for every service in the brief (each as its own slug folder with index.html and clean URL), following the same fixed layout and hub-and-spoke internal linking back to the homepage. EVERY page has a services section with a card per service — each card needs its OWN real image; generate every one with the generate_image tool (autonomously, no user setup) and make sure the files exist, so NO page ships with missing/broken service images. Then build the Privacy Policy and Terms & Conditions pages using the playbook's simplified legal layout. Commit and push.`,
  ];
  ...
  steps.push(
    `Final pass per the playbook, across ALL pages${bilingual ? " in BOTH English and Spanish" : ""}: SEO meta tags + JSON-LD schema + sitemap.xml (include every page URL${bilingual ? ", English and Spanish" : ""}) + robots.txt, adaptive favicons, the OG/social share image, the booking widget embed (if provided), and a 404 page. Generate any needed hero/section/OG images per the playbook's image rules — every generated image is a CLEAN PHOTOGRAPH with NO text, logos, brand names, or watermarks baked in (branding goes on via HTML/CSS only); use the generate_image tool for photos and generate_logo ONLY for the logo/favicon mark. The logo belongs ONLY in the header/footer at normal small size — NEVER use it as a hero/about/service/banner/OG image, never substitute it as a placeholder when a photo is missing, and NEVER place a large/faded/oversized copy of the logo or business name as a decorative watermark behind the hero text or behind any section; every image slot must have its own distinct real photograph. Before finishing, verify no image outside the header/footer references the logo file, and CRUCIALLY verify that EVERY \`<img src>\` and CSS \`background-image\` pointing at images/… resolves to a real file that exists on disk — walk every page (homepage, every service page, legal pages${bilingual ? ", and all Spanish pages" : ""}) and especially every services-section card; if any image file is missing, generate it now with generate_image so NO page ships with broken or missing images. Verify every internal link resolves${bilingual ? ", the EN⇄ES language toggle works both ways on every page, and hreflang/canonical tags are correct" : ""}, the no-pricing and contact-display rules hold on every page, and the site is deployable as a static site. Fix anything broken, then commit and push.`,
  );
